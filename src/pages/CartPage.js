...
// import { IncreaseItemCountInCartAction,DecreaseItemCountInCartAction} from "../redux/cartAction"
...
function CartPage() {
  ...
  const deleteFromCart = (product) => {
   ...
   const increaseItem = (item) => {
    dispatch({type: 'Increase_Item_Count_Cart',
    payload: item});
};

 const decreaseItem = (item) => {
   dispatch({type: 'Decrease_Item_Count_Cart',
    payload: item});
}
  ...
  const placeOrder = async () => {
    ...
    try {
      setLoading(true);
      const result = await addDoc(collection(fireDB, "dishes"), orderInfo);
      setLoading(false);
